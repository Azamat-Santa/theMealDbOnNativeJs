const URL_MEAL_NAME = 'https://www.themealdb.com/api/json/v1/1/search.php?s='
const section__meal= document.querySelector('.section__meal')
let category__arr = []
const mealName = localStorage.getItem('searchName')

console.log(mealName);
const getAllCategory = async () => {
  const req = await fetch(URL_MEAL_NAME + mealName)
  const resp = await req.json()
  category__arr = resp.meals
  category__arr ? category__arr.map(item => {
  ...
  section__meal.innerHTML=`<div style="margin: 10px auto; width:50%;">К сожалению блюда "${mealName}" на нашем сайте нету </div>`
}
getAllCategory()